...
void mNoThreadBeforeGameRoutine(struct mCoreThread* threadContext)
{
	//struct mCoreThread* threadContext = context;
	pthread_once(&_contextOnce, _createTLS);
	pthread_setspecific(_contextKey, threadContext);
...
void mNoThreadGameRoutine(struct mCoreThread* threadContext) 
{
	//struct mCoreThread* threadContext = context;
	struct mCoreThreadInternal* impl = threadContext->impl;
	struct mCore* core = threadContext->core;
...
void mNoThreadClosephase(void* context) 
{
	...
	//if (threadContext->logger.d.filter == &filter) {
	//	mLogFilterDeinit(&filter);
		mLogFilterDeinit(threadContext->logger.d.filter);
	//}
	threadContext->logger.d.filter = NULL;
}
...
static THREAD_ENTRY _mCoreThreaddummyRun(void* context)
{
	struct mCoreThread* threadContext = context;
#ifdef USE_PTHREADS
	pthread_once(&_contextOnce, _createTLS);
...
bool mCoreThreadBeginNoStart(struct mCoreThread* threadContext) {
	threadContext->impl = calloc(sizeof(*threadContext->impl), 1);
	threadContext->impl->state = THREAD_INITIALIZED;
	threadContext->logger.p = threadContext;
	if (!threadContext->logger.d.log) {
	...
	ThreadCreate(&threadContext->impl->thread, _mCoreThreaddummyRun, threadContext);
