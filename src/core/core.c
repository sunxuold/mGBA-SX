...
//sunxu added 
void mCoreTakeScreenshotWithName(struct mCore* core, int fileNo) {
#ifdef USE_PNG
	size_t stride;
	const void* pixels = 0;
	unsigned width, height;
	...
	struct VFile* vf;
	char path[PATH_MAX];
	snprintf(path, PATH_MAX - 1, "%s-%d.png", core->dirs.baseName, fileNo);
	path[PATH_MAX - 1] = '\0';
#ifndef PSP2
    //vf = VFileOpen( path, O_CREAT | O_TRUNC | O_WRONLY) 
	vf = core->dirs.screenshot->openFile(core->dirs.screenshot, path, O_CREAT | O_TRUNC | O_WRONLY);
	//vf = VDirFindNextAvailable(core->dirs.screenshot, core->dirs.baseName, "-", ".png", O_CREAT | O_TRUNC | O_WRONLY);
#else
	vf = VFileMemChunk(0, 0);
#endif
