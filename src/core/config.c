...
void mCoreConfigLoadDefaults(struct mCoreConfig* config, const struct mCoreOptions* opts) {
	...
	ConfigurationSetIntValue(&config->defaultsTable, 0, "interframeBlending", opts->interframeBlending);	
	ConfigurationSetValue(&config->defaultsTable, "", "savegamePath", opts->savegamePath);	
	ConfigurationSetValue(&config->defaultsTable, "", "savestatePath", opts->savestatePath);	
	ConfigurationSetValue(&config->defaultsTable, "", "screenshotPath", opts->screenshotPath);	
	ConfigurationSetValue(&config->defaultsTable, "", "patchPath", opts->patchPath);	
	ConfigurationSetValue(&config->defaultsTable, "", "cheatsPath", opts->cheatsPath);
