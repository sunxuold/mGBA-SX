...
bool mCheatParseEZFChtFile(struct mCheatDevice* device, struct VFile* vf) {
	...
	while (true) {
		...
		if (cheat[0] == '[') {
			...
			if((end - cheat-1) < (MAX_LINE_LENGTH-1))
			{
				strncpy(cheatName, &cheat[1], end - cheat-1);
				//cheatName[end - cheat-1]='';
			}
			else
			{
			   strncpy(cheatName, &cheat[1], MAX_LINE_LENGTH - 1);
		       //cheatName[MAX_LINE_LENGTH - 2]=' ';
			}
			   
			//printf("get cheatName %s!\n", cheatName);
			//free(name);
		...
		//printf("parseName is: %s!\n",set->name);		
		mCheatAddSet(device, set);		
		
		//struct mCheatSet*  cheatsss = (struct mCheatSet*)mCheatSetsGetPointer(&(device->cheats),cheatpos);
		//printf("getcheatsets:!\n");
		//cheatpos++;		
		//printf("after add Name is: %s!\n",cheatsss->name);
