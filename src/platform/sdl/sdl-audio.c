...
FILE *fp = NULL;
int currentsamples;
unsigned lastsoundframe=0, tempsoundframe = 0;
unsigned  soundframe = 0, minframe=0;
extern int showFPSinformation ;
extern int debugtest; 
struct timeval last, tval; // timing
...
bool mSDLInitAudio(struct mSDLAudio* context, struct mCoreThread* threadContext) {
	...
	context->desiredSpec.samples = context->samples;  //2frame = 44100*2/60 =1470 use= 1472
	currentsamples = context->samples;
	...
	fprintf(fp, "desiredSpec info: freq %d  format %d channels%d  silence %d samples %d  padding %d size %d \n",
		context->desiredSpec.freq,
		context->desiredSpec.format,
		context->desiredSpec.channels,
		context->desiredSpec.silence,
		context->desiredSpec.samples,
		context->desiredSpec.padding,
		context->desiredSpec.size);
	}
	//printf("desiredSpec samples %d size %d\n", context->desiredSpec.samples, context->desiredSpec.size);	
	...
	//printf("obtainedSpec samples %d size %d\n", context->obtainedSpec.samples, context->obtainedSpec.size);
	...
	if (debugtest)
	{
	fprintf(fp, "obtainedSpec info: freq %d  format %d channels%d  silence %d samples %d  padding %d size %d \n",
		context->obtainedSpec.freq,
		context->obtainedSpec.format,
		context->obtainedSpec.channels,
		context->obtainedSpec.silence,
		context->obtainedSpec.samples,
		context->obtainedSpec.padding,
		context->obtainedSpec.size);
	...
	/*
	if(context->obtainedSpec.samples > currentsamples)
	{
		context->obtainedSpec.samples = currentsamples;
		context->desiredSpec.samples = currentsamples;
	}
	*/
...
static void _mSDLAudioCallback(void* context, Uint8* data, int len) {
	...
	{
		tempsoundframe=sal_TimerRead(&tval, &last, 2);	
		if(lastsoundframe ==0 )
			lastsoundframe = tempsoundframe;
		else
		{
			if(soundframe <(tempsoundframe- lastsoundframe ))
				soundframe = tempsoundframe-lastsoundframe;
			
			if(minframe==0||(minframe >(tempsoundframe- lastsoundframe )))
				minframe = tempsoundframe-lastsoundframe;
...
/* 	
	if (debugtest)
	{
		...
		testflag++;			
		if(testflag==30||testflag==60||testflag==90||testflag==120)
			fprintf(fp,"init clockRate %d \n",clockRate); //sunxu test
	}
*/
...
/*	
	...
	{
		if(testflag==30||testflag==60||testflag==90||testflag==120)
			fprintf(fp, "clockRate %d  fauxClock %f \n",clockRate, fauxClock); 		
	}
*/	
...
/*	
	...
	{
	if(testflag==30||testflag==60||testflag==90||testflag==120)
		fprintf(fp, "available %d  len %d\n", available,len); //Sunxu add test				
	}
*/	
		//if(available>1029) 	printf("available %d \n", available);
		available = len;
	}
	...
	if (available < len) {
		...
		//if(available<512) printf("available %d \n", available);
		available = len;
		//sunxu test 
/*		
		if (debugtest)
		{
		if(testflag==30||testflag==60||testflag==90||testflag==120)
			fprintf(fp,"before  available %d  len %d\n", available, len); //Sunxu add test
		}		
*/		
		...
		//fprintf(fp,"after available %d  len %d\n", available, len);
	}
